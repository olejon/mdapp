...
public class InteractionsCardsActivity extends AppCompatActivity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		if(!searchString.contains("_"))
		{
			try
			{
				...
				JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, getString(R.string.project_api_uri)+"api/1/correct/?search="+URLEncoder.encode(searchString, "utf-8"), null, new Response.Listener<JSONObject>()
				{
					@Override
	...
	private void search(final String searchString)
	{
		try
		{
			...
			JsonArrayRequest jsonArrayRequest = new JsonArrayRequest(getString(R.string.project_api_uri)+"api/1/interactions/?search="+URLEncoder.encode(searchString, "utf-8"), new Response.Listener<JSONArray>()
			{
				@Override
					...
					public void onClick(View view)
					{
						if(mTools.isDeviceConnected())
						{
							try
							{
								...
								JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, getString(R.string.project_api_uri)+"api/1/interactions/handling/?uri="+URLEncoder.encode(uri, "utf-8"), null, new Response.Listener<JSONObject>()
								{
									@Override
									public void onResponse(JSONObject response)
									{
